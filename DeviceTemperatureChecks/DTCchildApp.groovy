    ...
    section("")
    {
        ...
        input name: "sendPushMessage", type: "capability.notification", title: "Notification Devices: Hubitat PhoneApp or Pushover", multiple: true, required: false, submitOnChange: true
       	if(sendPushMessage)
       	{
	    	...
	    	if(optionQuietTime)
   			{
   				input (name: "startQuietTime", type: "time", title: "Select Start Time?", defaultValue: false, required: true, width:3)
   				input (name: "endQuietTime", type: "time", title: "Select End Time?", defaultValue: false, required: true, width:3)
   			}
   			paragraph "<hr>"
        }
